...
void LowPass<T>::setAlpha(T value) {
    if (value < static_cast<T>(0) || value > static_cast<T>(1)) EXCEPTION("Smoothing coefficient must be in the range [0, 1]");
    alpha_ = value;
    alpha_defined_ = true;
}
...
T LowPass<T>::filter(T input) {
    if (!alpha_defined_) EXCEPTION("Smoothing coefficient has not been set or is invalid.");
